...
# Database configuration for trace business system
...
# Initialize database connection and source
trace_conn=Mssql_Conn(dbc_trace_business)
trace_source=Mssql_Source(dbc_trace_business)

# SQL query to create temporary table for packaging data
...
# Execute query and create temporary table
trace_conn.open()
cursor=trace_conn.execute(sql_str)

# Query to fetch transfusion order data with logistics and details
...
# Store transfusion data in dictionary
column_dict=trace_conn.column_dict()
transfusion_data_dict={}

# Process transfusion order data
row=cursor.fetchone()
while row:
...
# Query to fetch product batch quality data
sql_str="""
select distinct
pbw.ProdBatchID,
ws.WarehouseStockID,
...
# Process product batch quality data
trace_conn.open()
cursor=trace_conn.execute(sql_str)
column_dict=trace_conn.column_dict()
...
# Query to fetch PBMC (Peripheral Blood Mononuclear Cell) data
sql_str="""select 
v1.SaleOrderNo,
v1.ProdBatchID,
V1.ProdBatchCode,
...
# Query to fetch production batch detail data
sql_str="""select  
v1.ProdBatchCode,
v4.ProcName,
...
# Process production batch detail data
trace_conn.open()
cursor=trace_conn.execute(sql_str)
column_dict=trace_conn.column_dict()
...
# Query to fetch production batch data and combine with related data
trace_conn.open()
...
# Combine production batch data with PBMC, Product, and Detail information
row=cursor.fetchone()
while row:
    row_dict=dict(zip(column_dict,row))
...
# Query to fetch apheresis order data
trace_conn.open()
cursor=trace_conn.execute("""SELECT 
       [SaleOrderNo]
      ,[DanCaiNo]
...
# Process apheresis order data and combine with production batch data
row=cursor.fetchone()
while row:
    row_dict=dict(zip(column_dict,row))
...
# Query to fetch sale order mapping data
sql_str="""
SELECT [SaleOrderNo]
      ,[SaleOrderNoFix]
...
# Process sale order mapping data
while row:
...
# Query to fetch sale order confirmation dates
...
# Process sale order confirmation dates
while row:
    sale_order_confirm_date_dict[row[0]]=row[1]
    row=cursor.fetchone()
trace_conn.close()

# Query to fetch final sales order data with workflow information
trace_conn.open()
cursor=trace_conn.execute("""select 
...
# Process final sales order data and combine all related information
column_dict=trace_conn.column_dict()
row=cursor.fetchone()
data_list=[]
...
while row:
    ...
    # Combine apheresis and transfusion data for each mapped sale order
    ...
    # Determine confirmation date based on order type
    confirm_date=None
    if sale_order_data_dict["SaleOrderType"]=="常规订单":
        # For regular orders, use apheresis release date if quality status is appropriate
        ...
        # For scheduled orders, use either corrected confirm date or PBMC release date
